...
| 이메일/계정 식별자 | CLI 응답(`command.data` 등) 또는 CLI가 자체 기록한 실행 로그(`cli.log`)의 인증 이벤트에서 이메일을 추출하여 최대 80자의 `accountLabel`로 전달한다. 외부 키체인이나 비밀 토큰은 직접 읽지 않는다. |
...
- 계정 식별자는 공식 CLI의 응답 또는 CLI 실행 로그의 인증 성공 이벤트(`server_oauth`)에서만 비침습적으로 읽으며, OS 자격 증명 관리자의 암호화된 토큰이나 개인 설정을 직접 열지 않는다.
