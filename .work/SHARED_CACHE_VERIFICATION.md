...
- 기존 release exe는 수정 전 산출물이고 preview exe와 해시가 다르다. 재점검에서 기존 release exe는 다른 프로세스가 사용 중이며 preview exe는 교체 가능한 상태였다. 실행 중인 프로세스의 이미지 경로는 OS 조회에서 반환되지 않아 경로를 직접 확정하지는 못했다.
- OS 시간대는 한국 표준시였고, 공유 캐시의 Claude 항목은 최근 수집 성공 시각으로 갱신되어 있었다. 오래된 stale 캐시만의 문제로 단정하지 않는다. 수정된 파서의 회귀 테스트 4개는 다시 통과했다.
- 우선 기존 앱을 트레이에서 종료한 뒤 preview 수정본을 실행해야 한다. 기존 경로·바로가기를 계속 사용하려면 종료 후 preview exe를 기존 release exe에 복사하여 두 경로의 산출물을 일치시킨다. 실행 중인 사용자 앱을 강제 종료하거나 캐시 시간을 임의 보정하지 않았다.
- 수정본으로 실제 수집에 성공한 뒤에도 오차가 남는지는 사용자 확인이 필요하다. 이 경우 실행 경로와 수집 성공 여부, 계정정보를 제외한 reset 문구를 기준으로 추가 진단한다. 실계정 수집은 실행하지 않았다.
